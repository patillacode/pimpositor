...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), ('db/pimpositor'))

# Images configuration
MAX_CONTENT_LENGTH = 16 * 1024 * 1024  # 16 MB
UPLOAD_FOLDER = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ('static/usr_src_img')
)
...
def allowed_file(filename):
    ...
    """
    app.logger.info("Checking if file is allowed...")
...
def generate_and_save_uuid_to_db():
    ...
    uuid = str(uuid4())
    db = open_db()
    app.logger.info("Generating UUID...")
    if uuid in db:
        app.logger.info("UUID already exists, generating another one...")
...
def save_img(request):
    ...
    """
    app.logger.info("Getting picture from POST")
        ...
        if image and allowed_file(image.filename):
            ...
            app.logger.info(
                "Saving image to disk at {0}/{1}.{2}".format(
                    app.config['UPLOAD_FOLDER'], uuid, extension
                )
            )
            path = os.path.join(
                app.config['UPLOAD_FOLDER'], "{0}.{1}".format(uuid, extension)
            )
            try:
                image.save(path)
            except:
                app.logger.error("There was a problem saving the image.")
            ...
            try:
                ...
                app.logger.error("There was a problem opening the image.")
                app.logger.error(traceback.format_exc())
                return {"status": False}
            ...
            app.logger.info("Data does not meet requirements. Check the size.")
            return {"status": False}
    else:
        app.logger.info("No data was uploaded.")
        return {"status": False}
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if saved:
            ...
            app.logger.info("Rendering pimpositor...")
...
if __name__ == '__main__':
    parser = PimpParser()
    parser.add_argument(
        "--host", default="127.0.0.1", help="IP to run on [default: 127.0.0.1]"
    )
    parser.add_argument(
        "--port", default=7070, type=int, help="port to listen to [default: 7070"
    )
    parser.add_argument(
        "--debug", default=False, help="to set debug mode on [default: False]"
    )
